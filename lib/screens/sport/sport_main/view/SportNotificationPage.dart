...
class SportNotificationPage extends StatefulWidget {
  final String notificationPostID;


  const SportNotificationPage({
    Key? key,
    required this.notificationPostID,
  }) : super(key: key);

  @override
  _SportNotificationPageState createState() => _SportNotificationPageState();
}

class _SportNotificationPageState extends State<SportNotificationPage> {
  ...
  void initState() {
    ...
    sportPostHandler.init().then((_) {
      if (mounted) { // Added mounted check
        sportPostHandler
            .handlePostList(
            true, true, widget.notificationPostID)
            .then((_) {
          if (mounted) { // Added mounted check
            setState(() {});
          }
